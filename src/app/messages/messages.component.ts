...
import { SendStateDisplayComponent } from './send-state-display/sent-state-display.component';
...
export class MessagesComponent implements OnInit {
  ...
  private listenToNewMessage(): void {
    this.sseService.receiveNewMessage
      .pipe(
        filter(
          (newMessage) =>
            this.conversation?.publicId === newMessage.conversationId
        )
      )
  ...
  private listenToNavigateToConversation(): void {
    this.conversationService.navigateToConversation.subscribe(
      (conversation) => {
      ...
      if (message.sendDate) {
        if (message.sendDate.isSame(dayjs(), 'day')) {
          ...
          this.pushMessageWithKey(
            message,
            message.sendDate.format('DD/MM/YYYY')
          );
        }
      }
    }
      ...
      .pipe(
        filter(
          (conversationPublicId) =>
            this.conversation?.publicId === conversationPublicId
        )
      )
  ...
  private listenToMarkAsRead(): void {
    this.conversationService.markAsViewed.subscribe(
      (messageMarkAsReadState) => {
        if (
          messageMarkAsReadState.status === 'OK' &&
          messageMarkAsReadState.value
        ) {
          this.updateMessageSendState(
            messageMarkAsReadState.value.nbMessagesUpdated
          );
        }
      }
    );
  ...
  private updateMessageSendState(nbMessagesUpdated: number) {
    const messagesInReceivedState = this.conversation?.messages.filter(
      (message) => message.state === 'RECEIVED'
    );
    if (
      messagesInReceivedState &&
      nbMessagesUpdated === messagesInReceivedState.length
    ) {
