...
export class MessageService {
  ...
  handleSend(newMessage: Message): void {
    ...
    this.http
      .post<void>(`${environment.API_URL}/messages/send`, formData)
      .subscribe({
  ...
  extractSender(
    conversationUsers: Array<BaseUser>,
    senderId: string
  ): BaseUser {
    const indexSender = conversationUsers.findIndex(
      (user) => user.publicId === senderId
    );
