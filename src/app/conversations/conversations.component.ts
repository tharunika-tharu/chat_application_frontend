...
export class ConversationsComponent implements OnInit, OnDestroy {
  ...
  private listenToSSEDeleteConversation(): void {
    this.deleteSSESub = this.sseService.deleteConversation.subscribe(
      (uuidDeleted) => {
        const indexToDelete = this.conversations.findIndex(
          (conversation) => conversation.publicId === uuidDeleted
        );
  ...
  private listenToSSENewMessage(): void {
    this.sseService.receiveNewMessage.subscribe((newMessage) => {
      const indexToUpdate = this.conversations.findIndex(
        (conversation) => conversation.publicId === newMessage.conversationId
      );
      if (indexToUpdate === -1) {
        ...
        const sender = this.messageService.extractSender(
          conversationToUpdate.members,
          newMessage.senderId!
        );
        if (
          this.oauth2Service.fetchUser().value!.publicId !== sender.publicId
        ) {
          this.toastService.show(
            `New message received from ${sender.firstName} ${sender.lastName}`,
            'SUCCESS'
          );
        }
      }
      this.conversationService.sortConversationByLastMessage(
        this.conversations
      );
    });
  }
  ...
  private listenToSSEViewMessage(): void {
    this.viewedMessageSSESub = this.sseService.viewMessages.subscribe(
      (conversationViewedForNotification) => {
        if (
          this.selectedConversation?.publicId ===
          conversationViewedForNotification.conversationId
        ) {
          conversationViewedForNotification.messageIdsViewed.forEach(
            (messageId) => {
              const messageToUpdate = this.selectedConversation?.messages.find(
                (message) => message.publicId === messageId
              );
              if (messageToUpdate) {
